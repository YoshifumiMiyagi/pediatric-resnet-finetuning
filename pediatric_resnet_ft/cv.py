...
def default_transforms(image_size=224):
    ...
    train_tf = transforms.Compose([
        transforms.Resize((image_size, image_size)), transforms.RandomRotation(5),
...
def _run_epoch(model, loader, criterion, device, num_outputs, optimizer=None):
    ...
    with context:
        for images, labels in loader:
            ...
            if num_outputs == 1:
                ...
                raise ValueError("Binary classification supports only 1 or 2 outputs.")
    ...
    return (sum(losses) / len(loader.dataset),
            ...
            np.asarray(targets), np.asarray(probabilities))
...
def run_cv(
    ...
    reset_head: bool = False, device: Optional[str] = None,
):
    """Repeated patient-level CV with per-seed OOF predictions.

    Adult checkpoints automatically retain either a 1-logit BCE head or a 2-logit
    CrossEntropy head. For 2-logit models, OOF probability is softmax(logits)[:, 1].
    """
    ...
    output = Path(output_dir); weights_dir = output / "weights"
    output.mkdir(parents=True, exist_ok=True); weights_dir.mkdir(parents=True, exist_ok=True)
    ...
    for seed in seeds:
        ...
        for fold, (train_idx, valid_idx) in enumerate(split_iter):
            ...
            model = build_resnet50(pretrained=pretrained, pretrained_path=pretrained_path,
            ...
            if num_outputs == 1:
                ...
                raise ValueError(f"Expected 1 or 2 outputs for binary classification, got {num_outputs}.")
            ...
            for epoch in range(1, epochs + 1):
                ...
                val_loss, val_auc, _, _ = _run_epoch(model, valid_loader, criterion, device, num_outputs)
                scheduler.step()
                history_rows.append({"seed": seed, "fold": fold, "epoch": epoch,
                                     "train_loss": train_loss, "train_auc": train_auc,
                                     "val_loss": val_loss, "val_auc": val_auc, "num_outputs": num_outputs})
                score = val_auc if not np.isnan(val_auc) else -np.inf
                if score > best_auc:
                    best_auc = score; torch.save(model.state_dict(), best_path)

            model.load_state_dict(torch.load(best_path, map_location=device))
            val_loss, val_auc, _, val_prob = _run_epoch(model, valid_loader, criterion, device, num_outputs)
            seed_oof[valid_idx] = val_prob; seed_folds[valid_idx] = fold
            fold_row = {"seed": seed, "fold": fold, "n_train_images": len(train_idx),
                        "n_valid_images": len(valid_idx), "auc": val_auc, "loss": val_loss,
                        "num_outputs": num_outputs, "best_weight": str(best_path)}
            if effective_group_col is not None:
    ...
    fold_df = pd.DataFrame(fold_rows); history_df = pd.DataFrame(history_rows); seed_df = pd.DataFrame(seed_rows)
    ...
    return {"oof": oof_df, "fold_metrics": fold_df, "seed_metrics": seed_df,
            "history": history_df, "ensemble_oof_auc": ensemble_auc}
